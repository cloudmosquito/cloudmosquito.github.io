...
例如：[git 相关操作](../环境配置/git 用法.md#3-ssh){target="_blank"}
```
...
效果如下：[git 相关操作](../环境配置/git 用法.md#3-ssh){target="_blank"} 。
...
```markdown
![](./相机标定.assets/相机成像示意图1.png){width=50%}
```
...
此外，还有一种更好的方法，能进一步给图片加上题注：

```markdown
<figure markdown>
![](./相机标定.assets/相机成像示意图1.png){width=60%}
<figcaption>相机成像示意图</figcaption>
</figure>
```
