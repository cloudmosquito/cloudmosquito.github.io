...
以下内容面向熟悉C++但完全没接触过ROS的读者，逐行、逐概念地说明这段代码在做什么，以及所用到的ROS和MAVROS相关函数、消息类型、参数含义等。为了避免冗长重复，整体结构分为两部分：先介绍ROS与MAVROS基础概念，然后对代码逐行注释并补充必要的改进建议。
...
![](./实例—PX4降落移动平台.assets/image-1.png){.img-center width=50%}{.img-center width=50%}
...
参见 [服务（Service）](%E6%9C%8D%E5%8A%A1%EF%BC%88Service%EF%BC%89.md)
...
参见 [日志宏](%E6%97%A5%E5%BF%97%E5%AE%8F.md)
...
`msg` 指向发布方发送的位姿消息，包含 `msg->header`（时间戳 `msg->header.stamp`，坐标系 `msg->header.frame_id`）和 `msg->pose`（包括 `msg->pose.position.{x,y,z}` 以及 `msg->pose.orientation.{x,y,z,w}`）。
 ...
 用 `auto&` 获取对该最后航点对象的引用，以便修改其字段。假设最后一个航点是“着陆点”，所以后续将其坐标替换为动态目标坐标。
