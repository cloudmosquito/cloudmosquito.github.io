...
参见这篇 arxiv 上的 [tutorial](姿态表示的导数.assets/time_derivative_of_rotation_matrices.pdf)，写得很详细。
...
$$\dot{q} = \frac{1}{2}\begin{bmatrix}0\\ ^{\mathcal{W}}\boldsymbol\omega_{\mathcal{B}}\end{bmatrix}\otimes q \tag{3-1}$$

解释： $q$ 表示机体坐标系相对于固定坐标系的姿态， $^{\mathcal{W}}\boldsymbol\omega_{\mathcal{B}}$ 表示机体坐标系角速度在固定坐标系下的表示。

### 3.2 推导过程

我们考虑固定坐标系中的一个固定向量 $r$ ，其在机体坐标系下的表示为 $r_b$ ，则有

$$\begin{bmatrix}0\\ r\end{bmatrix} = q\otimes \begin{bmatrix}0\\r_b\end{bmatrix}\otimes q^* \tag{3-2}$$
...
$$\dot{q} \otimes q^* = \begin{bmatrix}v_0\\ \boldsymbol{v}\end{bmatrix} \tag{3-3}$$
...
所以 $v_0 = 0, \boldsymbol{v} = \dfrac{1}{2}(^{\mathcal{W}}\boldsymbol{\omega}_{\mathcal{B}})$ 。代回到（3-3）式，得到
