...
这里的 $x_0, y_0, z_0$ 是坐标系 0 的三个单位向量，$x_1, y_1, z_1$ 是坐标系 1 的三个单位向量。要求：坐标系 0 和坐标系 1 的 **原点重合**。

旋转矩阵的含义：

1. 三维坐标系 $o_1x_1y_1z_1$ 相对于参考系 $o_0x_0y_0z_0$ 的姿态。
2. 从三维坐标系 1 到三维坐标系 0 的旋转坐标变换。
3. 对坐标系 0 的一个旋转操作符。
...
!!! theorem "性质"
    ...
    $$\begin{aligned}
    \begin{bmatrix}\overset{\rightarrow}{\mathbf{x}_{A}} & \overset{\rightarrow}{\mathbf{y}_{A}} & \overset{\rightarrow}{\mathbf{z}_{A}}\end{bmatrix} &= \begin{bmatrix}\overset{\rightarrow}{\mathbf{x}_{B}} & \overset{\rightarrow}{\mathbf{y}_{B}} & \overset{\rightarrow}{\mathbf{z}_{B}}\end{bmatrix}{}^{A}_{B}R\\
    \begin{bmatrix}\overset{\rightarrow}{\mathbf{x}_{B}} & \overset{\rightarrow}{\mathbf{y}_{B}} & \overset{\rightarrow}{\mathbf{z}_{B}}\end{bmatrix} &= \begin{bmatrix}\overset{\rightarrow}{\mathbf{x}_{A}} & \overset{\rightarrow}{\mathbf{y}_{A}} & \overset{\rightarrow}{\mathbf{z}_{A}}\end{bmatrix}{}^{B}_{A}R
    \end{aligned}$$
...
以 X-Y-Z 顺序为例，内旋 X-Y-Z 的旋转矩阵是 $R_{x}(\phi)R_{y}(\theta)R_{z}(\psi)$ ，外旋 X-Y-Z 的旋转矩阵是 $R_{z}(\psi)R_{y}(\theta)R_{x}(\phi)$ .

“右乘连体” 的理解：将旋转矩阵视为旋转操作符，从左到右依次旋转连体坐标系。

“左乘基” 的理解： ${}^{0}\alpha = {}^{0}R_{1}{}^{1}\alpha = R_{x}(\phi){}^{1}\alpha$ 代表有两个坐标系，一个参考坐标系 0 ，一个坐标系 1 ，坐标系 1 相当于参考坐标系 0 绕自身 $x$ 轴旋转了 $\phi$ 角度。向量 ${}^{1}\alpha$ 是表示在坐标系 1 下的，现经过旋转矩阵处理，表示在坐标系 0 下，成为了 ${}^{0}\alpha$ 。

“左乘基”的过程相当于上述过程的重复， $R_{z}(\psi)\left[R_{y}(\theta)\left[R_{x}(\phi){}^{1}\alpha\right]\right]$ 的每个括号都相当于把向量 ${}^{1}\alpha$ 在坐标系 0 下表示。
