...
链式法则同样适用于矩阵微积分，只需要注意矩阵相乘的维度对应关系即可。实例参见
...
$$\begin{aligned}
\mathrm{d} \left(\mathbf{a}^\top\mathbf{b}\mathbf{a}\right) &= \mathrm{d} \left(\mathbf{a}^\top\mathbf{b}\right)\mathbf{a} + \left(\mathbf{a}^\top\mathbf{b}\right)\mathrm{d}\mathbf{a}\\
&= \left((\mathrm{d}\mathbf{a}^\top)\mathbf{b} + \mathbf{a}^\top(\mathrm{d}\mathbf{b})\right)\mathbf{a} + \left(\mathbf{a}^\top\mathbf{b}\right)\mathrm{d}\mathbf{a}\\
