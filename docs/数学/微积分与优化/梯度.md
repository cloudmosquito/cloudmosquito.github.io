...
$$\nabla f(\mathbf{x}) = \begin{pmatrix}\dfrac{\partial f}{\partial x_1}(\mathbf{x})\\ \dfrac{\partial f}{\partial x_2}(\mathbf{x})\\ \vdots\\ \dfrac{\partial f}{\partial x_n}(\mathbf{x})\end{pmatrix}$$

我们考虑在点 $\mathbf{x}$ 处做一阶泰勒展开，有

$$\begin{aligned}
f(\mathbf{x}+\Delta\mathbf{x}) &= f(\mathbf{x})+df(\mathbf{x})\Delta\mathbf{x}+o(\left\|\Delta\mathbf{x}\right\|)\\
&=f(\mathbf{x})+ \begin{pmatrix}\dfrac{\partial f}{\partial x_1} & \cdots & \dfrac{\partial f}{\partial x_n}\end{pmatrix}\begin{pmatrix}\Delta x_1\\ \vdots\\ \Delta x_n\end{pmatrix} + o(\left\|\Delta\mathbf{x}\right\|)\\
...
给定可微函数 $f:\mathbb{R}^n \rightarrow \mathbb{R}$ ，在点 $\mathbf{x}$ 处，考察不同方向的函数值变化率：
