...
当标量函数 $V(x)$ 是二次型形式，也就是说，$V(x) = x^{\top}Ax$，其中$A$是一个线性定常矩阵时，我们也会说，$A$ 具有与 $V(x)$ 一致的 (半)正/负定性。 
### 2.1 二次型为什么要求矩阵对称？
...
!!! note
    ...
    $$x^{\top}Ax = \sum\limits_{i}\sum\limits_{j}a_{i,j}x_ix_j$$

    但它们同时也会要求 $A$ **是一个对称矩阵**，这是为什么呢？

    我们从字面意义上思考“二次型”这个概念，很自然会觉得，如果标量函数可以完全表示成完全平方形式的线性组合，那么它就是一个二次型。

    注意到，对于任意矩阵 $A$ ，$x^{\top}Ax$ 都是符合我们直觉的一个二次型。

    举个例子，$2x_1^2+2x_1x_2+2x_2^2 = x_1^2+(x_1+x_2)^2+x_2^2$，只要根据耦合项 $2x_1x_2$ 配系数，再留出单独的平方项 $x_1^2+x_2^2$，就能把任意一个二次型转化为完全平方的形式。

    那么可以合理怀疑，对任意矩阵 $A$ ，我们都可以把 $x^{\top}Ax$ 改写成矩阵为对称矩阵的 $x^{\top}Bx$ 形式，即 $\forall A, \exists B, B = B^{\top}, x^{\top}Ax = x^{\top}Bx$.

    事实上，我们也确实可以找到这样的一个$B$，$B = (A^{\top} + A)/2$.

    具体推导过程如下所示

    $$\begin{aligned}x^{\top}\left(\frac{A^{\top}+A}{2}\right)x &= \frac{1}{2}\left(x^{\top}A^{\top}x + x^{\top}Ax\right)\\ &= \frac{1}{2}\left[(x^{\top}Ax)^{\top}+x^{\top}Ax\right]\\ &= \frac{1}{2}\left(x^{\top}Ax+x^{\top}Ax\right)\\ &=x^{\top}Ax.\end{aligned}$$

    其中，第二行到第三行是因为 $x^{\top}Ax$ 是一个标量。
...
##### 【充分性】：$A_n \text{正定} \Rightarrow \Delta_k>0, 0<k\le n$ .
...
##### 【必要性】：$\Delta_k>0, 0<k\le n \Rightarrow A_n \text{正定}$。

用数学归纳法证明。

- 当 n=1 时，$\Delta_1 = a_1 > 0$，有 $x^TA_{(1)}x = a_1x^Tx = a_1x^2 > 0, x \neq 0$，所以 $A_{(1)}$ 是正定的；

- 当 n=2 时，$\Delta_1 = a_{11}>0, \Delta_2 = a_{11}a_{22}-a_{12}^2>0$，有
...
考虑一般的二次函数 $f:\mathbb{R}^n\rightarrow \mathbb{R}$ ，形式为

$$f(x)=x^\top Qx+b^\top x+c,$$

其中，$Q\in\mathbb{R}^{n\times n}$，$b\in\mathbb{R}^n$，$c\in\mathbb{R}$ 。方便起见，我们定义对称矩阵 $A=\dfrac{Q+Q^\top}{2}$ ，则 $f$ 可重新表示为

$$f(x)=x^\top Ax+b^\top x+c.$$

我们知道，函数上凹/下凹的定义为：

若对任意 $x\neq y\in\mathbb{R}^n$ 和任意 $t\in(0,1)$ ，都有

$$f(tx+(1-t)y)>tf(x)+(1-t)f(y),$$

则 $f$ 是上凹的。反之，若 $f(tx+(1-t)y)<tf(x)+(1-t)f(y)$，则 $f$ 是下凹的。

我们接下来证明：**$f$ 是上凹/下凹的，等价于二次项矩阵 $A$ 是负定/正定的**。

任取 $x,y\in\mathbb{R}^n$ ，令 $v=x-y\neq 0$ 。对 $t\in(0,1)$ ，$x-y$ 线段上的点为
...
$$\begin{aligned}
f(tx+(1-t)y)=&f(y+tv)\\
=&y^\top Ay+t^2v^\top Av+2tv^\top Ay+b^\top y+tb^\top v+c\\
=&f(y)+t^2v^\top Av+2tv^\top Ay+tb^\top v\\
tf(x)+(1-t)f(y)
=&tf(y+v)+(1-t)f(y)\\
=& ty^\top Ay+tv^\top Av+2ty^\top A v+tb^\top(y+v)+tc\\
&+(1-t)y^\top Ay+(1-t)b^\top y+(1-t)c\\
=&f(y)+tv^\top Av+2ty^\top A v+tb^\top v.
\end{aligned}$$

两式相减，得

$$f(tx+(1-t)y)-[tf(x)+(1-t)f(y)]=(t^2-t)v^\top Av.$$

由于 $t\in(0,1)$ ，因此 $t^2-t<0$ 。因此当 $A$ 负定时，$f$ 是上凹的。$\blacksquare$ 