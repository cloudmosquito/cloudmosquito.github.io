...
一个特征向量线性无关（所有特征值都不同）的 $n$ 维矩阵 $A$，可以写成如下形式
...
> 这里还有一个观察：如果 $\mathbf{x}_i, \ i=1,2,...,n$ 两两正交，单位长度，那么，矩阵 $B = \begin{bmatrix}\mathbf{x}_1 & \mathbf{x}_2 & \cdots & \mathbf{x}_n\end{bmatrix}^{-1} = \begin{bmatrix}\mathbf{x}_1 & \mathbf{x}_2 & \cdots & \mathbf{x}_n\end{bmatrix}^T$ 。这时候的 $B$ 就是一个酉矩阵。
