...
$$\begin{aligned}
[\phi_1, \phi_2] &= (\Phi_1\Phi_2 - \Phi_2\Phi_1)^{\vee}, \Phi = \hat{\phi}\in\mathbb{R}^{3\times3}\\
...
&= \phi \phi^{T} - \begin{bmatrix}\phi_1^2+\phi_2^2+\phi_3^2 & 0 & 0\\ 0 & \phi_1^2+\phi_2^2+\phi_3^2 & 0\\ 0 & 0 & \phi_1^2+\phi_2^2+\phi_3^2\end{bmatrix}
\end{aligned}\tag{2}$$

$$\hat{\phi}\phi\cdot\phi^T = \begin{bmatrix}0 & -\phi_3 & \phi_2\\
\phi_3 & 0 & -\phi_1\\
...
因此，我们想到一个思路，那就是把 $\phi$ 用模长 $\theta$ 和方向 $\mathbf{a}$ 来表示，即 $\phi = \theta \mathbf{a}$ . 这里的 $\mathbf{a}$ 是个长度为 1 的方向向量，所以 $\hat{\mathbf{a}} \hat{\mathbf{a}} = \mathbf{a} \mathbf{a}^T - \mathbf{I}$，$\hat{\mathbf{a}} \hat{\mathbf{a}} \hat{\mathbf{a}}  = -\hat{\mathbf{a}} $ .
...
&= (\mathbf{a} \mathbf{a}^T -\hat{\mathbf{a}} \hat{\mathbf{a}} )+\theta\hat{\mathbf{a}}  + \frac{1}{2}\theta^2\hat{\mathbf{a}} \hat{\mathbf{a}} -\frac{1}{3!}\theta^3\hat{\mathbf{a}} -\frac{1}{4!}\theta^4\hat{\mathbf{a}} \hat{\mathbf{a}} \\
&= \mathbf{a} \mathbf{a}^T + \hat{\mathbf{a}} (\theta - \frac{1}{3!}\theta^3 + \frac{1}{5!}\theta^5 + ...) + \hat{\mathbf{a}} \hat{\mathbf{a}} (-1 + \frac{1}{2}\theta^2-\frac{1}{4}\theta^4 + ...)\\
&= \mathbf{a}\mathbf{a}^T + \hat{\mathbf{a}} \sin\theta + \hat{\mathbf{a}} \hat{\mathbf{a}} (-\cos\theta)\\
&= \mathbf{a} \mathbf{a}^T + \hat{\mathbf{a}} \sin\theta + (\mathbf{I}-\mathbf{a} \mathbf{a}^T)\cos\theta\\
&= \mathbf{I}\cos\theta + (1-\cos\theta)\mathbf{a} \mathbf{a}^T + \hat{\mathbf{a}} \sin\theta
\end{aligned}$$

$$\mathbf{R} = \sum_{n = 0}^{\infty}\frac{1}{n!}(\hat{\phi})^{n} = \mathbf{I}\cos\theta + (1-\cos\theta)\mathbf{a} \mathbf{a}^T + \hat{\mathbf{a}} \sin\theta \tag{4}$$
...
$$\mathbf{R} = \cos\theta \mathbf{I} + (1-\cos\theta)\mathbf{n}\mathbf{n}^T + \sin\theta \hat{\mathbf{n}}\tag{5}$$
...
$$\hat{\mathbf{a}}\hat{\mathbf{a}} = \mathbf{a}\mathbf{a}^T - \mathbf{I} \tag{6}$$
...
$$\phi = \ln(\mathbf{R})^{\vee} = \left(\sum_{n=0}^{\infty} \frac{(-1)^{n}}{n+1}(\mathbf{R} - \mathbf{I})^{n+1} \right)^{\vee}\tag{8} $$

当然我们不会真的用 (8) 式计算 $\phi$，这是个无穷级数，我们无法求解。我们可以利用 (7) 式对其进行求解。
...
$$\theta = \arccos\dfrac{\text{tr}(\mathbf{R})-1}{2} \tag{9}$$

接下来，我们需要求解旋转矩阵对应的的等轴角表示时的转轴，我们给出两种方法。

**第一种方法**：可以注意到，转轴是旋转过程中的一个不变量，因此有

$$\mathbf{R}\mathbf{a} = \mathbf{a} \tag{10}$$
...
$$\begin{aligned}
\mathbf{R}^T &= \mathbf{I} + \hat{\mathbf{a}}^T \sin\theta + \left(\hat{\mathbf{a}}^T\right)^2(1-\cos\theta) \\
...
$$\begin{aligned}
\mathbf{R} - \mathbf{R}^T &= 2\hat{\mathbf{a}}\sin\theta\\
\hat{\mathbf{a}} &= \frac{\mathbf{R}-\mathbf{R}^T}{2\sin\theta}
\end{aligned} \tag{12}$$

综上，$\Phi = \dfrac{\theta}{2\sin\theta}(\mathbf{R} - \mathbf{R}^T)$，$\theta = \arccos\dfrac{\text{tr}(\mathbf{R})-1}{2}$，$\phi = \check{\Phi}$ 。
