...
> 【注意】一般 DQN（以及 Q-learning）只能处理动作离散的情况，因为函数 Q 更新过程中有 $\max_{a}$ 的操作。
...
实际应用的时候一般是所有 (s,a) 的价值都减去状态 s 时的动作优势函数平均值。
...
这里的 $\eta,\alpha,\beta$ 都表示网络参数。可见，$V$ 和 $A$ 会共用一部分隐藏层。上式的另一个好处是满足
