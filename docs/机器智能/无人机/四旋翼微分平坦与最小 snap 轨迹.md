...
本次阅读的论文为：D. Mellinger and V. Kumar, "Minimum snap trajectory generation and control for quadrotors," 2011 IEEE International Conference on Robotics and Automation, Shanghai, China, 2011, pp. 2520-2525.
...
![](./四旋翼微分平坦与最小 snap 轨迹.assets/%E5%9D%90%E6%A0%87%E7%B3%BB.png){.img-center width=50%}
...
首先是**梯度下降法**。我们先确定一个 $\mathbf{T}$ 的初值，然后以此出发点进行搜索。怎么搜索呢？直观的想法就是每个“方向”我都去看一看，综合判断一下哪个方向最好。所以我们定义一组方向变量 $g_i$，满足第 i 个分量是 1，其他所有分量都是 $\frac{-1}{m-1}$ 。为什么要这么做？因为这样就能保证，$\mathbf{T} + g_i$ 后，仍然能够保证各个时间段求和得到的总时间为 $t_m$ 。
