...
上一节的讨论限定在 $\mathrm{SO}(3)$ 纯旋转。当引入平移后，齐次变换矩阵 $T = \begin{bmatrix}R&t\\0&1\end{bmatrix}$ 的乘法规也同样有"右乘联体左乘基"的解读，但有细微差异。

首先注意到 $T$ 可以分解为旋转与平移的乘积，且顺序不可交换：

$$T = \begin{bmatrix}R&t\\0&1\end{bmatrix} = \begin{bmatrix}I&t\\0&1\end{bmatrix} \begin{bmatrix}R&0\\0&1\end{bmatrix} \neq \begin{bmatrix}R&0\\0&1\end{bmatrix} \begin{bmatrix}I&t\\0&1\end{bmatrix}$$

也就是说，"右乘联体"是先在联体坐标系平移，再旋转；"左乘基"是先在基坐标系旋转，再平移。

具体看四个组合运算：

$$\begin{aligned}
\begin{bmatrix}R&0\\0&1\end{bmatrix}\begin{bmatrix}R_0&t_0\\0&1\end{bmatrix} &= \begin{bmatrix}RR_0&Rt_0\\0&1\end{bmatrix}\\
\begin{bmatrix}I&t\\0&1\end{bmatrix}\begin{bmatrix}R_0&t_0\\0&1\end{bmatrix} &= \begin{bmatrix}R_0&t_0+t\\0&1\end{bmatrix}\\
\begin{bmatrix}R_0&t_0\\0&1\end{bmatrix}\begin{bmatrix}R&0\\0&1\end{bmatrix} &= \begin{bmatrix}R_0R&t_0\\0&1\end{bmatrix}\\
\begin{bmatrix}R_0&t_0\\0&1\end{bmatrix}\begin{bmatrix}I&t\\0&1\end{bmatrix} &= \begin{bmatrix}R_0&R_0t+t_0\\0&1\end{bmatrix}
\end{aligned}$$

分析：

1. 左乘平移（第二行）与右乘旋转（第三行）显然满足口诀——前者在基坐标系下位移，后者在联体坐标系下旋转；
2. 右乘平移（第四行）中 $R_0t+t_0$ 本质是在联体坐标系下平移了 $t$ 再在基坐标系下表示，满足口诀；
3. 左乘旋转（第一行）不完全满足口诀——不仅在基坐标系下发生旋转，还附带平移了 $Rt_0-t_0$。
...
参见这篇 arxiv 上的 [tutorial](1609.06088v1.pdf) ，写得很详细。

#### 例子

一架无人机在世界坐标系下的期望姿态为 $R_B^{W} = \begin{bmatrix}^{W}\mathbf{x}_B & ^{W}\mathbf{y}_B & ^{W}\mathbf{z}_B\end{bmatrix}$，且已知三轴向量导数，如何求解它的期望角速度？

答：由 $\dot{R}_B^W = R_B^W [^B\omega]^{\wedge}$ ，可得 $[^B\omega]^{\wedge} = \left(R_B^W\right)^{\top}\dot{R}_B^W$，即

$$

\begin{bmatrix}0 & -\omega_z & \omega_y\\
\omega_z & 0 & -\omega_x\\ -\omega_y & \omega_x & 0\end{bmatrix}=\begin{bmatrix}\mathbf{x}^{\top} \\ \mathbf{y}^{\top}\\ \mathbf{z}^{\top}\end{bmatrix}\begin{bmatrix}\dot{\mathbf{x}} & \dot{\mathbf{y}} & \dot{\mathbf{z}}\end{bmatrix}
...
$$\begin{aligned}
\omega_x &= \frac{1}{2}\left(\mathbf{z}^{\top}\dot{\mathbf{y}}-\mathbf{y}^{\top}\dot{\mathbf{z}}\right)\\
\omega_y &= \frac{1}{2}\left(\mathbf{x}^{\top}\dot{\mathbf{z}}-\mathbf{z}^{\top}\dot{\mathbf{x}}\right)\\
\omega_z &= \frac{1}{2}\left(\mathbf{y}^{\top}\dot{\mathbf{x}}-\mathbf{x}^{\top}\dot{\mathbf{y}}\right)
\end{aligned}$$

!!! question

    Q : 为什么不能直接 $\omega_x = \mathbf{z}^{\top}\dot{\mathbf{y}}$ ，而要相加取平均？

    A : 因为实际计算过程中有浮点误差累积等问题，尤其是三轴向量导数，取平均在数值上更加鲁棒。
...
容易混淆的两个概念：**固定角**（Fixed Angle）是将运动分解为每次绕固定参考坐标系轴的旋转；**欧拉角**（Euler Angle）是将运动分解为每次绕联体坐标系轴的旋转。

一句话总结：**固定角左乘，欧拉角右乘。**

举例：
- 固定角 ${}_B^AR_{XYZ}(\alpha,\beta,\gamma) = R_Z(\gamma)R_Y(\beta)R_X(\alpha)$
- 欧拉角 ${}_B^AR_{XY'Z''}(\alpha, \beta, \gamma) = R_X(\alpha)R_{Y'}(\beta)R_{Z''}(\gamma)$

欧拉角的旋转轴右上角带" $\prime$ "，表示当前轴与原来的轴不是同一条——$Y'$ 是经历了一次 $R_X(\alpha)$ 后的 $Y$ 轴，而非固定坐标系里的 $Y$。固定角不用加撇，因为旋转轴始终未变。
