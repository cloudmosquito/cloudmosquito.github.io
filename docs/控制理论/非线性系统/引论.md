...
考虑线性时不变系统 $\dot{x} = Ax$，且 矩阵 $A$ 可逆且特征值互异。

这时候可以把矩阵 $A$ 进行对角化，有

$$M^{-1}AM = J$$

这里的 $M$ 是 $A$ 的特征向量组成的矩阵，$J$ 是由 $A$ 的特征值组成的对角阵。
...
由于 $M^{-1}$ 是时不变的，所以可以做变量替换 $z = M^{-1}x$ 得到
...
$$z_1 = z_{10}e^{\lambda_1t}, z_2 = z_{20}e^{\lambda_2t}$$

消去 $t$ 后我们就得到了两个状态变量的关系：

$$z_2 = (\frac{z_1}{z_{10}})^{\lambda_2/\lambda_1}z_{20}$$

以上方法仅适用于特征值均为实数的情况，若特征值是一对共轭复数
