...
先令 $K_m$ 为 0 ，求通解（零输入分量）。该形式的微分方程解的形式为 $Ce^{at}$ ，代入可求得 $a = -\dfrac{1}{T}$ ，因此零输入分量为 $y_{zi}(t) = Ce^{-\dfrac{t-kT_s}{T}}$ . 再求特殊解（零状态分量）。观察可知 $y_{zs}(t) = K_m = Kx(kT_s)$ 即为一特殊解。因此，解为

$$y(t) = y_{zi}(t)+y_{zs}(t) = Ce^{-\dfrac{t-kT_s}{T}}+Kx(kT_s)$$

考虑 $t=kT_s$ 时，输出信号为 $y(kT_s)$ ，代入可解出 $C$ .

$$y(t) = y_{zi}(t)+y_{zs}(t) = y(kT_s)e^{-\dfrac{t-kT_s}{T}}+Kx(kT_s)\left(1-e^{-\dfrac{t-kT_s}{T}}\right)$$

输出信号是连续的，我们可以用上式求 $t=(k+1)T_s$ 时的输出信号

$$y[(k+1)T_s] = y(kT_s)e^{-\dfrac{T_s}{T}}+Kx(kT_s)\left(1-e^{-\dfrac{T_s}{T}}\right)$$

上式已经是一个离散的差分方程，等价于

$$y[k+1] = e^{-\dfrac{T_s}{T}}y[k] + K\left(1-e^{-\dfrac{T_s}{T}}\right)x[k]$$
